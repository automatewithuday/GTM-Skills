...
description: "Use when the opening line should quote a live job posting that names the prospect's own problem (keywords like \"cold calling\" or \"Salesforce\" in the description), with freshness gates so aged-out postings are blanked. Also use when hiring intent should filter the list."
license: MIT
