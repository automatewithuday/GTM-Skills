...
description: Use when running a monthly sweep for champions or heavy users of paying customers who changed jobs — building per-account watchlists, verifying moves against live LinkedIn data, routing qualifying moves into account scoring as MQLs, and flagging churn risk on the account they left. Also use when a job-change signal fired a false MQL (same-name profile, stale headline, or a free-trial user treated as an advocate). Never sends outreach.
license: MIT
metadata:
...
Replace every `{{...}}` before enabling. See the [setup checklist](references/setup-checklist.md) for the full list.
