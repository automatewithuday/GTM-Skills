...
description: "Use when learning cold email end to end or launching a first campaign the manual way: buying domains (Dynadot), provisioning inboxes (Zapmail), building lists (Prospeo), writing copy, enrichment, and sending (Smartlead or Instantly). Also use as the reference library when another skill needs deliverability, sequence, API, legal, or troubleshooting detail."
license: MIT
...
If you already have inboxes set up and just want a campaign to run automatically, invoke [`/auto-research-public`](../auto-research-public/SKILL.md). It does all of this in one flow: scrapes the target company → Claude generates an ICP → Prospeo pulls leads → emails are verified → Claude personalizes each lead via parallel Task sub-agents → Smartlead campaign launches. ~20 minutes end-to-end, no skill-juggling.

Prerequisites for the fast path:
- Ran [`/icp-onboarding`](../icp-onboarding/SKILL.md) once (produces `client-profile.yaml`)
- Ran [`/zapmail-domain-setup-public`](../zapmail-domain-setup-public/SKILL.md) + [`/smartlead-inbox-manager`](../smartlead-inbox-manager/SKILL.md) to have active inboxes
- Have `SMARTLEAD_API_KEY`, `PROSPEO_API_KEY`, `MILLIONVERIFIER_API_KEY` in `.env`

The walkthrough below is for **manual** first-timer learning. If you've done a campaign once before, jump straight to [`/auto-research-public`](../auto-research-public/SKILL.md).
...
**If you're new to cold email:** run [`/cold-email-kickoff`](../cold-email-kickoff/SKILL.md) instead of going through this tutorial manually. It orchestrates the entire setup (ICP + lead magnet + strategy + plan) in one flow.
