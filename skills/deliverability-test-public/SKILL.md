...
description: "Use when comparing reply, bounce, and positive-reply rates by inbox type (SMTP vs Gmail vs Outlook) across a Smartlead account, or deciding which inbox provider to buy more of. Also use when a mixed inbox fleet shows inconsistent results and the provider is the suspected variable."
license: MIT
...
- **Reply rate here is RAW, not positive.** For positive reply rate (the more important metric), use [`/positive-reply-scoring`](../positive-reply-scoring/SKILL.md).
...
**If one inbox type is underperforming:** [`/smartlead-inbox-manager`](../smartlead-inbox-manager/SKILL.md) to retire the bad ones (tag `retired`, disable warmup). If replacements are needed, [`/zapmail-domain-setup-public`](../zapmail-domain-setup-public/SKILL.md) to provision new domains on a different provider type.

**If all types are healthy:** back to the weekly rhythm via [`/cold-email-weekly-rhythm`](../cold-email-weekly-rhythm/SKILL.md).
...
- [`/email-deliverability-audit`](../email-deliverability-audit/SKILL.md) — full audit (SPF/DKIM/DMARC + reputation + spam placement)
- [`/positive-reply-scoring`](../positive-reply-scoring/SKILL.md) — the metric that matters, not just reply rate
- [`/smartlead-inbox-manager`](../smartlead-inbox-manager/SKILL.md) — rotate out bad inboxes, tag by performance
