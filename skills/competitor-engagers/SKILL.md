...
After collecting the engager CSV, sample ~50 rows and run [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md) before scaling further. LinkedIn engagement alone doesn't guarantee ICP fit — someone who reacted to a competitor's post might be a peer, a candidate, a student, or an actual buyer. Qualifying filters separates them.

**Why required:** unfiltered engager lists typically have 30-50% non-ICP rows (fans, peers, recruiters, students). Running [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md) on a 50-row sample, tuning the qualification prompt, then applying it to the full list cuts wasted enrichment + send costs. Takes 10-15 min.
...
**Run [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md)** on a 50-row sample of the engagers CSV (required step — LinkedIn engagement alone doesn't guarantee ICP fit). Then [`/list-quality-scorecard`](../list-quality-scorecard/SKILL.md) on the filtered list.
...
- [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md) — required filter pass on this skill's output
...
- [`/campaign-copywriting`](../campaign-copywriting/SKILL.md) — write copy referencing their specific engagement pattern
