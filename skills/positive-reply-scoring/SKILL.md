...
description: "Use when judging whether a Smartlead campaign is actually working: pull replies, classify them (interested, soft positive, referral, neutral, negative, OOO, bounce, unsubscribe), and compute positive reply rate against the 1% bar. Also use when raw reply rate looks fine but pipeline is not showing up."
license: MIT
...
**Respond to every `positive_interested` reply within 30 seconds** of seeing it. Then [`/experiment-design`](../experiment-design/SKILL.md) to plan the next iteration based on what worked.

**If positive reply rate is <1% after 200+ sends:** the 1% rule failed. Run [`/email-deliverability-audit`](../email-deliverability-audit/SKILL.md) (are you reaching the inbox?) (check for vague CTAs, generic first lines, em dashes).

**Or wait:** this skill is the Wednesday task in [`/cold-email-weekly-rhythm`](../cold-email-weekly-rhythm/SKILL.md). Run it weekly going forward.

## Related skills

- [`/experiment-design`](../experiment-design/SKILL.md) — uses positive reply rate as the success metric
- [`/email-deliverability-audit`](../email-deliverability-audit/SKILL.md) — if hostile + unsub are elevated, run this next
- [`/cold-email-starter-kit`](../cold-email-starter-kit/SKILL.md) → `10-reply-handling.md` for what to do with the positive replies once flagged
