...
- If any campaign failed the 1% rule: run [`/deliverability-incident-response`](../deliverability-incident-response/SKILL.md) → triage decision tree
...
3. Decide:
   ...
   - **Middling (near baseline):** iterate on copy or list — run [`/experiment-design`](../experiment-design/SKILL.md) Monday to plan the next variant
...
**Critical:** do not skip the log. Over a quarter, these logs become the input for [`/experiment-design`](../experiment-design/SKILL.md) quarterly reviews. Without the history, you can't learn across campaigns.
...
**If insurance pool is getting thin (<5 inboxes available to rotate in):** kick off a new domain purchase cycle via [`/zapmail-domain-setup-public`](../zapmail-domain-setup-public/SKILL.md). It takes ~2 weeks from purchase to sendable, so you need to start early.

---

## Monthly (1st of the month) — Spam placement test (25 min active, test runs ~20 min)

**Run:** the Smart Delivery spam placement test from [`/email-deliverability-audit`](../email-deliverability-audit/SKILL.md):
...
- **<80%:** red. Pause the campaign, run [`/deliverability-incident-response`](../deliverability-incident-response/SKILL.md), fix before sending more.
...
Use the retrospective as input to design next quarter's experiments via [`/experiment-design`](../experiment-design/SKILL.md).
...
- [`/email-deliverability-audit`](../email-deliverability-audit/SKILL.md) — the Monday and Monthly tasks
- [`/positive-reply-scoring`](../positive-reply-scoring/SKILL.md) — the Wednesday and Friday tasks
- [`/smartlead-inbox-manager`](../smartlead-inbox-manager/SKILL.md) — the biweekly inbox rotation
- [`/deliverability-incident-response`](../deliverability-incident-response/SKILL.md) — when the Monday audit flags something
- [`/experiment-design`](../experiment-design/SKILL.md) — the quarterly retrospective feeds into this
- [`/zapmail-domain-setup-public`](../zapmail-domain-setup-public/SKILL.md) — when insurance pool runs low
