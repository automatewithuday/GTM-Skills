...
description: "Use when launching a cold email campaign from a single target company domain with no list, ICP, or copy yet, Smartlead inboxes are already warmed, and the goal is a live campaign in under an hour with minimal hand-holding. Also use when someone wants the whole scrape-to-upload pipeline automated rather than run skill by skill."
license: MIT
...
Before running:
- [ ] `client-profile.yaml` exists (run [`/icp-onboarding`](../icp-onboarding/SKILL.md) if not)
- [ ] `SMARTLEAD_API_KEY` in env
- [ ] `PROSPEO_API_KEY` in env
- [ ] `MILLIONVERIFIER_API_KEY` in env (for email validation)
- [ ] At least 20 Smartlead inboxes tagged "active" (run [`/smartlead-inbox-manager`](../smartlead-inbox-manager/SKILL.md) first)
...
Rules Claude MUST follow (run [`/spam-word-checker`](../spam-word-checker/SKILL.md) on output):
...
Claude fans out to parallel Task sub-agents (one per variant × batch of 20-30 leads). See [`/personalization-subagent-pattern`](../personalization-subagent-pattern/SKILL.md) for the full pattern.
...
This is your experiment log. [`/experiment-design`](../experiment-design/SKILL.md) reads from here to compare runs. [`/positive-reply-scoring`](../positive-reply-scoring/SKILL.md) writes results back to this file after 21 days.
...
- **Prospeo INVALID_FILTERS** — Usually "industry name not in the 256 list." Check [`/icp-onboarding`](../icp-onboarding/SKILL.md) references/prospeo-industries.md for exact matches.
...
- **Sub-agent personalization repetitive** — If you see the same phrasing across leads, rerun that batch with a diversity prompt. See [`/personalization-subagent-pattern`](../personalization-subagent-pattern/SKILL.md) references/failure-modes.md.
...
**Wait 21 days** for the campaign to accumulate reply data, then run [`/positive-reply-scoring`](../positive-reply-scoring/SKILL.md) on the launched campaign.

**Meanwhile:** continue the weekly rhythm via [`/cold-email-weekly-rhythm`](../cold-email-weekly-rhythm/SKILL.md). Every Monday, [`/email-deliverability-audit`](../email-deliverability-audit/SKILL.md) on the new campaign to catch infrastructure issues early.
...
- [`/icp-onboarding`](../icp-onboarding/SKILL.md) — produces client-profile.yaml (required input)
- [`/lead-magnet-brainstorm`](../lead-magnet-brainstorm/SKILL.md) — produces the offer/CTA this campaign asks about
- [`/personalization-subagent-pattern`](../personalization-subagent-pattern/SKILL.md) — the fan-out pattern used in phase 6
- [`/smartlead-inbox-manager`](../smartlead-inbox-manager/SKILL.md) — must run BEFORE so inboxes are tagged/warmed
- [`/positive-reply-scoring`](../positive-reply-scoring/SKILL.md) — run AFTER 21 days to score the campaign
- [`/experiment-design`](../experiment-design/SKILL.md) — how to plan which target to try next
