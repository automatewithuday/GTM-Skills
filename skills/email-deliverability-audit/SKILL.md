...
description: "Use when reply rates drop, bounces spike, a Smartlead account is being taken over, or a routine weekly health check is due: SPF/DKIM/DMARC checks, inbox reputation, spam placement, and domains under the 1% reply-rate line. Also use before scaling send volume."
license: MIT
...
- Copy is broken (manually review for vague CTAs, generic openers, or em dashes; re-run [`/spam-word-checker`](../spam-word-checker/SKILL.md))
...
- Missing DKIM / SPF → [`/zapmail-domain-setup-public`](../zapmail-domain-setup-public/SKILL.md) to reconnect domains through Zapmail
- Blocked inboxes → [`/smartlead-inbox-manager`](../smartlead-inbox-manager/SKILL.md) to tag "retired" and rotate in insurance
- Campaign schedule issues → Smartlead campaign schedule settings
- Bad copy flagged → [`/spam-word-checker`](../spam-word-checker/SKILL.md) on the campaign copy
...
**If any flag fired:** [`/deliverability-incident-response`](../deliverability-incident-response/SKILL.md) → triage decision tree for whatever was flagged (low reply rate, high bounce, blocked inbox, etc).

**If all clean:** next Monday, run this again. This audit is the Monday task in [`/cold-email-weekly-rhythm`](../cold-email-weekly-rhythm/SKILL.md).

**Or wait:** if you just applied fixes, wait 7 days then re-audit. Reputation changes propagate slowly.

## Related skills

- [`/smartlead-inbox-manager`](../smartlead-inbox-manager/SKILL.md) — execute the action items (rotate, retag, warmup settings)
- [`/zapmail-domain-setup-public`](../zapmail-domain-setup-public/SKILL.md) — fix DNS/auth issues at the domain provider
- [`/spam-word-checker`](../spam-word-checker/SKILL.md) — check copy for spam-triggering phrases
- [`/deliverability-test-public`](../deliverability-test-public/SKILL.md) — lighter-weight SMTP vs Gmail vs Outlook reply/bounce comparison
