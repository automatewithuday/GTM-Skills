...
description: "Use when a lead list is enriched and about to be uploaded to Smartlead or another sender, and it needs a grade on duplicates, title relevance, bad-title patterns, catch-all density, verification coverage, and ICP fit first. Also use when a campaign underperformed and the list is the suspect."
license: MIT
...
Optional `--icp-file` lets the scorecard compare your list against your declared ICP filters (from [`/icp-onboarding`](../icp-onboarding/SKILL.md)).
...
- AFTER list-building skills ([`/prospeo-full-export`](../prospeo-full-export/SKILL.md), [`/blitz-list-builder`](../blitz-list-builder/SKILL.md), [`/google-maps-list-builder`](../google-maps-list-builder/SKILL.md), [`/disco-like`](../disco-like/SKILL.md))
...
**If grade ≥ B:** [`/campaign-copywriting`](../campaign-copywriting/SKILL.md) to write the emails. Then [`/smartlead-campaign-upload-public`](../smartlead-campaign-upload-public/SKILL.md) to launch as DRAFT.
...
- [`/icp-prompt-builder`](../icp-prompt-builder/SKILL.md) — more surgical ICP fit scoring (AI per-company)
- [`/icp-onboarding`](../icp-onboarding/SKILL.md) — produces the `client-profile.yaml` this skill checks against
- `/email-waterfall` — run BEFORE this skill for verification coverage

## The 1% rule alignment

A list that scores below C-grade is very likely to produce reply rates below 1%, which violates the 1% rule (see [`/email-deliverability-audit`](../email-deliverability-audit/SKILL.md)). Catching list issues here saves you the deliverability hangover later.
